...
# Output directory
output_dir = '/home/gs285/DBLP_scrape/conference_sum/visualization_university'

# Function to create bar chart for each conference
def create_conference_chart(data, conference, output_dir):
    universities = list(data.keys())
    year_ranges = ["2020-2024", "2014-2019"]
    
    latest_counts = [data[uni][year_ranges[0]].get(conference, 0) for uni in universities]
    previous_counts = [data[uni][year_ranges[1]].get(conference, 0) for uni in universities]
    ...
    bars1 = ax.bar(x - width/2, latest_counts, width, label='2020-2024', color='skyblue')
    bars2 = ax.bar(x + width/2, previous_counts, width, label='2014-2019', color='salmon')
    
    # Add some text for labels, title, and custom x-axis tick labels, etc.
    ax.set_xlabel('University', fontsize=14)
    ax.set_ylabel('Number of Papers', fontsize=14)
    ax.set_title(f'{conference} Paper Counts by University (2020-2024 vs 2014-2019)', fontsize=16)
    ax.set_xticks(x)
    ...
    # Save the figure
    output_path = f"{output_dir}/{conference}_conference_comparison.png"
    plt.savefig(output_path)
    plt.close()

    print(f"Saved plot for {conference} at {output_path}")

# Generate and save bar charts for each conference
for conference in conferences:
    create_conference_chart(data, conference, output_dir)